...
2. **无效 hex fallback**：`pickerHex` 对未输完 hex 回退 `#ffffff`，面板指针突变
3. **主题强制改色**：`coerceWidgetSurfaceBackground` / `normalizeStyleConfigForColorScheme` 在深色主题下将非暗色组件底强制改为 `#1e293b`，用户显式选色被覆盖
...
- `fe/src/components/ui/color-field.tsx`：popover 打开或防抖 pending 时不同步 prop；面板内变更立即 commit 合法 hex；仅提交 `normalizeHexColor` 成功的值
