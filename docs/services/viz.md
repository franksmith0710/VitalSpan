...
- **`gis-map` 散点叠加**（FE `fe/src/components/charts/engine/maplibre/gisMapOverlay*.ts`）：经/纬 + 可选指标 → MapLibre GeoJSON `setData`；官方示例 Dataset **`demo-map-scatter`**（`sample_db.de_map_heat`）；区域着色仍用 `map`/`map-3d` + `demo-v-sales-geo`
