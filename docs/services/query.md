...
- **LIMIT 取最新**：翻译 SQL 时若选出时间列（如 `sale_date` / `updated_at`），先 `ORDER BY 时间 DESC LIMIT N`，再按时间升序返回给图表；无时间列则仍为无序前 N 行。组件「结果展示」预设 10/20/50/100/500/1000（默认 **1000**，与 `query_default_limit` 对齐），历史 `all`/`10000` 视为 1000。
