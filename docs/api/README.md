...
| 方法 | 路径 | 说明 | IF | 期次 | PRD | 状态 | 代码锚点 |
|------|------|------|-----|------|-----|------|----------|
| GET/POST | `/api/v1/dashboards` | Dashboard 列表/创建；列表返回 `previewSummary`+`widgetCount`+`surfaceKind`+`thumbnailUrl`（无完整 `layoutJson`）；可选 `surfaceKind` 过滤；SQL 分页 | 内部 | 一期 | DASH-001 | 已实现 | `backend/app/api/v1/dashboards.py` |
