...
4. bundle：`host.vsCv.mount(`；样式 `(p && p.style) || {}`；**六块**须消费 `labelShow`/`tooltipShow`/`seriesGradient`/`paletteColors` 或 `--vs-palette-*`（`BUNDLE-BOILERPLATE.md` §7 `resolveStyle`）；布局读 `p.layout` 缩放，溢出用组件内滚动（见 §4）；**勿**在 styleSchema 重复 `maxItems`/刷新/六块（见 `STYLE-SCHEMA.md`）。
