...
- 多类别外部数据源（关系型、OLAP、时序、文档、搜索等）**缺乏统一接入与分析入口**
...
2. **G2 — 多类别数据源可配置接入**：关系型、OLAP、时序、文档、搜索等通过连接器插件接入，**新增类型不改核心框架**（NFR-04）；支持**库表级同步入平台托管分析库**与**轻量清洗**（FR-DATA/FR-ETL，里程碑 M1B）。
...
| 多源统一接入 | 连接器插件 + 可选同步/清洗入托管分析库（M1B），再经 `dataSourceId` 查询 |
...
- 自研 M1~M8 全模块（按 SRS §8 四期分期交付，里程碑细节见 plan.md）
