...
- **演化建议**：二期扩展健康检查维度（DB 连通性）；`tests/test_health.py` 已覆盖 CORS 预检与 OpenAPI 公开路径
...
- **演化建议**：二期补 TanStack Query、`@/lib/api.ts` 与业务页；`fe/src/routes.smoke.test.tsx` vitest 已覆盖 `/admin` 壳层挂载
...
- **演化建议**：`tests/test_migrations.py` 已覆盖 Settings 与 `migrations/env.py` URL 绑定；CI 仍不跑 docker `alembic upgrade`；M1B 增 ingestion 元表 revision
...
- **演化建议**：CI 已含 backend 11 pytest + frontend vitest；二期增 docker postgres job 与 Playwright E2E
