...
- **验收标准**：
  ...
  - [x] **列表轻量真实预览**（2026-08-07）：看板/大屏列表始终渲染真实布局（`previewProfile=card`）；真实 `query/execute` 但单图 ≤50 行、关闭标签/图例/tooltip/钻取/动画；全局 live ≤3 并发（`dashboardPreviewProfile` · `listPreviewActivation`）
