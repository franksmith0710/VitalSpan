...
- **全球 PMTiles（外部服务）**：管理员在元库登记 `tileServiceId`（`viz_tile_services`）；前端经 `GET /api/v1/tile-services/{id}/resolve` 获取 PMTiles URL；**128GB 级制品不进主 release**，仅内网 HTTP Range 静态服务或 `go-pmtiles serve`
