...
describe("chartFontSizes", () => {
  it("covers 10–24 step 1 and 26–48 even steps", () => {
    expect(CHART_FONT_SIZE_OPTIONS[0]).toBe(10);
    expect(CHART_FONT_SIZE_OPTIONS[14]).toBe(24);
    expect(CHART_FONT_SIZE_OPTIONS[15]).toBe(26);
    expect(CHART_FONT_SIZE_OPTIONS.at(-1)).toBe(48);
    expect(CHART_FONT_SIZE_OPTIONS).toHaveLength(27);
  });
