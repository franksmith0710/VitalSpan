...
import { DEMO_MAP_DRILL_SQL, DEMO_MAP_JOIN_SQL } from "@/lib/mapChartDataHint";

export type MapDataPresetId = "demo-sales-province" | "demo-drill-sql";

export const MAP_DATA_PRESET_LABELS: Record<MapDataPresetId, string> = {
  "demo-sales-province": "省级 · 演示库",
  "demo-drill-sql": "省→市→区县 · 演示",
};
...
export function detectMapDataPreset(cfg: ChartViewConfig): MapDataPresetId | null {
  ...
  if (dims[0] === "province" && dims[1] === "city") return "demo-drill-sql";
  if (dims[0] === "region" && cfg.sql?.includes("JOIN regions")) return "demo-sales-province";
  return null;
}
