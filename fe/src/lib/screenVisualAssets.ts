...
export type ScreenVisualInsertType = "screen-clock" | "screen-border";

export function isScreenVisualInsertType(
  type: string,
): type is ScreenVisualInsertType {
  return type === "screen-clock" || type === "screen-border";
}
...
export function isScreenVisualWidget(
  ...
  return isScreenClockWidget(widget) || isScreenBorderWidget(widget);
}
