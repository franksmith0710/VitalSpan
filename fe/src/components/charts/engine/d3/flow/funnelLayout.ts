...
export function computeFunnelLayout(opts: {
  ...
  const extrude = Math.max(0, opts.extrudePx ?? 0);
  let margin = {
    top: FUNNEL_PAD.top + extrude,
    right: FUNNEL_PAD.right + (extrude > 0 ? extrude : 0),
    bottom: FUNNEL_PAD.bottom,
    left: FUNNEL_PAD.left,
  };
  ...
  const gap = Math.max(0, opts.gap ?? 4, extrude > 0 ? extrude + 2 : 0);
