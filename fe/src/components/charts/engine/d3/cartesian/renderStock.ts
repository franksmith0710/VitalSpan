          ...
          tooltip
            ...
            .html(
              `<div style="font-weight:600;margin-bottom:2px">${d.type}</div>` +
                `<div>å¼€ <strong>${formatChartValue(d.open, valueFormat)}</strong> Â· æ”?<strong>${formatChartValue(d.close, valueFormat)}</strong></div>` +
                `<div>ä½?<strong>${formatChartValue(d.low, valueFormat)}</strong> Â· é«?<strong>${formatChartValue(d.high, valueFormat)}</strong></div>`,
            );
        })
        .on("mousemove", (event) => {
