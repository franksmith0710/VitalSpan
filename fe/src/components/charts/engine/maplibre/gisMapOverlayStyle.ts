...
import { DEFAULT_GIS_OVERLAY, resolveGisOverlayStyle } from "@/components/charts/engine/maplibre/gisProject";
...
export function buildGisOverlayCircleRadius(resolved: ReturnType<typeof resolveGisOverlayStyle>): number | ExpressionSpecification {
...
export function buildGisOverlayLabelPaint(
  flavor: GisBasemapFlavor,
): Record<string, unknown> {
...
export function buildGisOverlayLayerDefinitions(
  ...
  return {
    source: {
      id: GIS_OVERLAY_SOURCE_ID,
      spec: { type: "geojson" as const, data: overlay },
    },
    circleLayer: {
    ...
    labelLayer: {
      ...
      paint: buildGisOverlayLabelPaint(options.flavor),
    },
  };
}
...
export function syncGisOverlayStyle(
  map: MapLibreMap,
  options: GisOverlayLayerOptions,
) {
  whenMapStyleReady(map, () => {
    if (!map.getLayer(GIS_OVERLAY_CIRCLE_LAYER_ID)) return;
    const resolved = resolveGisOverlayStyle(options.overlay, options.chartColors);
    const circlePaint = buildGisOverlayCirclePaint(resolved);
    for (const [key, value] of Object.entries(circlePaint)) {
      map.setPaintProperty(GIS_OVERLAY_CIRCLE_LAYER_ID, key, value);
    }
    ...
    const labelPaint = buildGisOverlayLabelPaint(options.flavor);
